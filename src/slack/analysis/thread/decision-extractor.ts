...
export class DecisionExtractor {
  ...
  private calculateDecisionConfidence(text: string): number {
    ...
    if (text.length > 20) {
      // Reduced threshold for Japanese
      confidence += 0.1;
    }

    // Formal language patterns (English and Japanese)
    if (
      text.includes('we have') ||
      text.includes('it has been') ||
      text.includes('team has') ||
      text.includes('しました') ||
      text.includes('ます') ||
      text.includes('決定')
    ) {
   ...
   */
  async extractDecisionsService(
    args: unknown
  ): Promise<ServiceResult<DecisionExtractionResult> & { statusCode: number }> {
    try {
      // Basic validation
