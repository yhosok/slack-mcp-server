 ...
 */
function deduplicateActivityPeriods(periods: Array<{
  startTime: number;
  endTime: number;
  messageCount: number;
  participants: string[];
  ...
  for (const current of sorted) {
    const hasOverlap = result.some(existing => 
      (current.startTime < existing.endTime && current.endTime > existing.startTime)
    );
    ...
    if (!hasOverlap) {
      ...
      if (overlappingIndex !== -1 && current.messageCount > result[overlappingIndex]!.messageCount) {
