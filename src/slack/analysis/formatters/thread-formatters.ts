...
export function formatThreadMetrics(
  ...
  if (options.includeAverages) {
    const averagesLines = [
      createListItem(
        `Messages per thread: ${formatNumber(metrics.avg_messages_per_thread, options.precision)}`
      ),
      createListItem(
        `Participants per thread: ${formatNumber(metrics.avg_participants_per_thread, options.precision)}`
      ),
      createListItem(
        `Duration: ${formatNumber(metrics.avg_duration_hours, options.precision)} hours`
      ),
    ];
    sections.push(
...
export function createThreadSummarySummary(
  ...
  return summaryText.length > maxLength ? summaryText.slice(0, maxLength - 3) + '...' : summaryText;
}
