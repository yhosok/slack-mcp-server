...
export function formatFileAnalysis(
  ...
  if (options.includeTypeBreakdown && Object.keys(analysis.by_type).length > 0) {
    ...
    const typeLines = typeEntries.map(([type, stats]) => 
      createListItem(`${type}: ${formatNumber(stats.count, 0)} files (${formatBytes(stats.size_bytes)})`)
    );
    
    sections.push(
      createSectionHeader('By File Type', options.includeEmojis ? '📁' : '', options.includeEmojis) + 
      typeLines.join('\n')
    );
  }
  ...
  if (options.includeUserStats && Object.keys(analysis.by_user).length > 0) {
    ...
    const userLines = userEntries.map(([user, stats]) => 
      createListItem(`${user}: ${formatNumber(stats.count, 0)} files (${formatBytes(stats.size_bytes)})`)
    );
    
    sections.push(
      createSectionHeader('Top Uploaders', options.includeEmojis ? '👤' : '', options.includeEmojis) + 
      userLines.join('\n')
    );
  }
  ...
  if (options.includeLargeFiles && analysis.large_files.length > 0) {
    const largeFileLines = analysis.large_files
      .slice(0, Math.min(5, options.maxItems))
      .map(file => 
        createListItem(`${file.name}: ${formatBytes(file.size)}`)
      );
 ...
 */
export function createFileAnalysisSummary(
  analysis: FileAnalysis,
...
export function formatChannelFileDistribution(
  ...
  const lines = channelEntries.map(([channel, stats]) => 
    createListItem(`${channel}: ${formatNumber(stats.count, 0)} files (${formatBytes(stats.size_bytes)})`)
  );
 ...
 */
export function formatFileSizeDistribution(
...
export function formatStorageUsageSummary(
  ...
  const lines = [
    ...
    createListItem(`Average File Size: ${formatBytes(analysis.total_files > 0 ? analysis.total_size_bytes / analysis.total_files : 0)}`)
  ];
  ...
  if (topTypes.length > 0 && topTypes[0]) {
    lines.push(createListItem(`Largest Type: ${topTypes[0][0]} (${formatBytes(topTypes[0][1].size_bytes)})`));
  }
