...
export const createReactionService = (deps: ReactionServiceDependencies): ReactionService => {
  ...
  const getReactionStatistics = (args: unknown) =>
    deps.requestHandler.handle(GetReactionStatisticsSchema, args, async (input) => {
      ...
      if (input.user) {
        allMessages = allMessages.filter((msg: any) => msg.user === input.user);
      }
      ...
      for (const message of allMessages) {
        if (message.reactions && message.reactions.length > 0) {
          ...
          const totalMessageReactions = message.reactions.reduce((sum: number, r: any) => sum + (r.count || 0), 0);
  ...
  const findMessagesByReactions = (args: unknown) =>
    deps.requestHandler.handle(FindMessagesByReactionsSchema, args, async (input) => {
          ...
          for (const channel of channels) {
            if (channel.id) {
              try {
                ...
                const messagesWithChannel = (historyResult.messages || []).map((msg: any) => ({
                  ...msg,
                  channel: channel.id,
      ...
      for (const message of allMessages) {
        ...
        const messageReactions = message.reactions.map((r: any) => r.name);
        const totalReactionCount = message.reactions.reduce((sum: number, r: any) => sum + (r.count || 0), 0);
