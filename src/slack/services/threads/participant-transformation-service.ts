...
export interface ParticipantTransformationService {
   ...
   */
  getEnhancedUserInfoForExport(userIds: string[]): Promise<
    ServiceResult<{
      userInfoMap: Record<
        string,
        {
...
export const createParticipantTransformationService = (
    ...
    try {
      ...
      try {
        ...
        const displayNameMap =
          await deps.infrastructureUserService.bulkGetDisplayNames(uniqueUserIds);

        // Get detailed user info for enhanced capabilities
        const userInfoPromises = uniqueUserIds.map(
          async (userId): Promise<[string, SlackUser | null]> => {
            try {
      ...
      return createServiceError(errorMessage, 'Failed to build participants from messages');
    }
  };
  ...
  > => {
    try {
      ...
      const enhancedUserInfoRecord: Record<
        string,
        {
          displayName: string;
          isAdmin?: boolean;
      ...
      return createServiceError(errorMessage, 'Failed to get enhanced user info for export');
    }
  };
