...
export class CacheService {
  ...
  getMetrics(): CacheServiceMetrics {
    ...
    const totalHits =
      channelMetrics.hits +
      userMetrics.hits +
      searchMetrics.resultHits +
      fileMetrics.hits +
      threadMetrics.hits;
    const totalMisses =
      channelMetrics.misses +
      userMetrics.misses +
      searchMetrics.resultMisses +
      fileMetrics.misses +
      threadMetrics.misses;
    ...
    const totalMemoryUsage =
      channelMetrics.memoryUsage +
      userMetrics.memoryUsage +
      searchMetrics.memoryUsage +
      fileMetrics.memoryUsage +
      threadMetrics.memoryUsage;
  ...
  getHealthStatus(): CacheHealthStatus {
    ...
    const memoryPressure = this.config.globalMemoryLimit
      ? memoryUsage / this.config.globalMemoryLimit > 0.9
      : false;
    ...
    return {
      ...
      performance: {
        averageResponseTime: this.performanceMonitor
          ? this.performanceMonitor.getPerformanceReport().averageResponseTime || 0
          : 0,
        memoryUsage,
  ...
  private async checkMemoryUsage(): Promise<void> {
    ...
    if (usagePercentage > 90) {
      logger.warn(
        `Cache memory usage high: ${usagePercentage.toFixed(1)}% (${currentUsage}/${limit} bytes)`
      );
   ...
   */
  private checkSearchCacheHealth(metrics: SearchCacheMetrics): {
    healthy: boolean;
    issues?: string[];
  } {
    const issues: string[] = [];
...
export class CachePerformanceMonitor {
  ...
  getPerformanceReport(): PerformanceReport {
    const recent = this.performanceData.filter(
      (d) => Date.now() - d.timestamp < 5 * 60 * 1000 // Last 5 minutes
    );
  ...
  private groupOperationsByType(
    data: Array<{ name: string; duration: number; timestamp: number }>
  ): Record<string, { count: number; average: number; min: number; max: number }> {
