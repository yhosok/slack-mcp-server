...
} from '../slack/infrastructure/validation/types.js';
...
describe('RequestHandler Type Safety Success (Green Phase)', () => {
  ...
  describe('TypeSafeAPI generic constraints implementation', () => {
    it('should enforce Record<string, any> constraint and reject primitive types', async () => {
      ...
      const validObjectOperation = async (_input: {
        test: string;
  ...
  describe('Service output types implementation', () => {
    it('should have well-defined deleteFile output type', async () => {
      ...
      const deleteFileOperation = async (_input: {
        file_id: string;
      }): Promise<{
        success: boolean;
        fileId: string;
    ...
    it('should have well-defined uploadFile output type', async () => {
      ...
      const uploadFileOperation = async (_input: {
        file_path: string;
        channels: string[];
      }): Promise<{
      ...
      await handler.handle(
        schema,
        { file_path: '/test.txt', channels: ['C123'] },
        uploadFileOperation
      );
    ...
    it('should have well-defined analyzeThread output type', async () => {
      ...
      await handler.handle(
        schema,
        { channel: 'C123', thread_ts: '1234567890.123' },
        analyzeThreadOperation
      );
  ...
  describe('TypeSafeAPI pattern compliance', () => {
    ...
    it('should enforce compile-time service output contracts', () => {
      ...
      const consistentService = async (_input: {
        test: string;
      }): Promise<{
        success: boolean;
        data: string;
  ...
  describe('Type system improvements with TypeSafeAPI patterns', () => {
    it('should prevent void operations in handle method through type constraints', async () => {
      ...
      const validOperation = async (_input: {
        action: string;
      }): Promise<{
        success: boolean;
        action: string;
    ...
    it('should handle properly structured async operations', async () => {
      ...
      const properOperation = async (_input: {
        id: string;
      }): Promise<{
        result: string;
        id: string;
